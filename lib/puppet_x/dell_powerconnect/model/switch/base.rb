...
module PuppetX::DellPowerconnect::Model::Switch::Base
  def self.register(base)
    ...
    base.register_model(:interface, PuppetX::DellPowerconnect::Model::Interface, /^interface\s+(\S+)\r*$/, 'sh run')
    base.register_model(:portchannel, PuppetX::DellPowerconnect::Model::Portchannel, /^interface(\s)+port-channel(.+)$/, 'show run')
